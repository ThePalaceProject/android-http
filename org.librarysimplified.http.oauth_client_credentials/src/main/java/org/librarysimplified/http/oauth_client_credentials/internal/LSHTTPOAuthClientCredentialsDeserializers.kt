...
class LSHTTPOAuthClientCredentialsDeserializers private constructor(
  ...
  private val logger =
    LoggerFactory.getLogger(LSHTTPOAuthClientCredentialsDeserializers::class.java)
  ...
  companion object {
    ...
    fun create(): LSHTTPOAuthClientCredentialsDeserializers {
      val allow = this.allowClasses()
      for (entry in allow) {
        this.logger.trace("whitelist: {}", entry)
      }
      return LSHTTPOAuthClientCredentialsDeserializers(allow)
    }
  }
}
